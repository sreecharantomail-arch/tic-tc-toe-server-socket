...
function _createSocketConnection() {
    ...
    socket.on('game:rematch-request', ({ from }) => {
        addSystemMessage(
            `${from === localSymbol ? 'You' : from === 'X' ? playerNames.X : playerNames.O} wants a rematch!`
        );
    });
...
function createPrivateRoom() {
    if (!player.name) {
        showXpNotification('Set a name first!');
        return;
    }
    if (!socket?.connected) {
        showXpNotification('Not connected to server!');
        return;
    }
    gameMode = 'room';
...
const _matchmakingTimeout = null; // keep for leaveGame cancel
...
function _onCellClick() {
    ...
    if (boardState.isOver || boardState.cells[idx]) {
        return;
    }

    if (gameMode === '2p') {
        _placeMarker(idx, boardState.currentTurn);
        return;
    }
    if (gameMode === 'ai') {
        if (boardState.currentTurn !== 'X') {
            return;
        }
        _placeMarker(idx, 'X');
        return;
    ...
    if (boardState.currentTurn !== localSymbol) {
        return;
    }
    if (!socket?.connected) {
        return;
...
function _runAiTurn() {
    if (boardState.isOver || boardState.currentTurn !== 'O') {
        return;
    }
...
function _recordOutcome(winnerSymbol) {
    ...
    if (gameMode === '2p') {
        ...
        if (aiDifficulty === 'hard' && gameMode === 'ai') {
            recordHardAiBeat();
        }
        awardXp(50);
        awardCoins(20);
...
function _renderTimer() {
    ...
    arc.className = `timer-arc${timerSeconds <= 5 ? ' critical' : timerSeconds <= 8 ? ' warning' : ''}`;
}
...
function _updateActivePlayerCards() {
    document.getElementById('cX').className =
        `player-card xc${boardState.currentTurn === 'X' ? ' ax' : ''}`;
    document.getElementById('cO').className =
        `player-card oc${boardState.currentTurn === 'O' ? ' ao' : ''}`;
}
...
function sendChat() {
    ...
    if ((gameMode === 'room' || gameMode === 'random') && socket?.connected) {
        ...
        if (Math.random() < 0.4) {
            ...
            const response =
                OPPONENT_CHAT_RESPONSES[Math.floor(Math.random() * OPPONENT_CHAT_RESPONSES.length)];
...
function sendEmojiReaction(emoji, triggerEvent) {
    ...
    if (Date.now() - lastEmojiTime < 1000) {
        return;
    }
