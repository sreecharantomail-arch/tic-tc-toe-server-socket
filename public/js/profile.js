...
function renderAchievementList() {
  ...
  if (countEl) countEl.textContent = player.unlockedAchievements.length;
  if (totalEl) totalEl.textContent = ACHIEVEMENTS.length;

  for (const ach of ACHIEVEMENTS) {
    const unlocked = player.unlockedAchievements.includes(ach.id);

    const rewardText = unlocked
      ? `+${ach.reward} coins earned`
      : `+${ach.reward} coins reward`;
    ...
    item.innerHTML = `
      ...
      ${
        unlocked
          ? '<div style="color:var(--green);font-size:1rem;">✓</div>'
          : ''
      }
    `;
...
function renderProfileScreen() {
  ...
  document.getElementById('profile-level-badge').textContent =
    `LEVEL ${player.level}`;
